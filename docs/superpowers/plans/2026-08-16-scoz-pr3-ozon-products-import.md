...
class ProductSnapshot: ...  # exact 45-field order from frozen PR3 Implementation Spec §3.1
...
@dataclass(frozen=True)
class OzonProductsImportSummary: ...  # exact 17-field DTO from frozen PR3 Implementation Spec §3.4
...
Errors are concrete and narrow: `UnsupportedWorkbook`, `WrongReportType`, `IncompatibleReportSchema`, `InvalidReportPeriod`, `InvalidProductIdentity`, `InvalidMetricValue`, `CategoryMismatch`, `ConflictingObservationRows`, `ConcurrentImportConflict`, `UploadTooLarge`, `UnsupportedUploadMediaType`, and `ImportPersistenceError`. `OzonProductsImportFailure` is the focused application-to-transport carrier: `error` is one of these narrow PR3 domain/application exceptions and `result` is `ImportResult | None`; its exact public attributes are `self.error` and `self.result`. Parser fatal errors expose no local path/content; recoverable errors become `RowError` with frozen user messages.
...
- [ ] Install updated development requirements from the repository root: `python -m pip install -r requirements-dev.txt`; expect exit code 0. This occurs after the RED proof and the `requirements.txt` change but before GREEN. Existing `requirements-dev.txt` already includes `-r requirements.txt`, so this one canonical command installs the exact runtime pins together with existing development/test dependencies; do not modify `requirements-dev.txt`.
...
- [ ] Write RED tests `test_product_snapshot_has_exact_frozen_field_order`, `test_ozon_products_import_summary_has_exact_frozen_field_order`, `test_decimal_from_excel_number_accepts_int_and_finite_float_only`, `test_canonical_decimal_text_is_plain_and_stable`, and `test_product_snapshot_payload_has_exact_keys_and_json_types`. Compare the exact ordered field-name lists—not only their counts—with the frozen spec: `[field.name for field in dataclasses.fields(ProductSnapshot)]` must contain exactly 45 names in the frozen order, and `[field.name for field in dataclasses.fields(OzonProductsImportSummary)]` must contain exactly 17 names in the frozen order. Instantiate all exact ProductSnapshot fields.
...
- [ ] Implement the exact 45-field order from frozen PR3 Implementation Spec §3.1 and exact 17-field DTO from frozen PR3 Implementation Spec §3.4. Add the report/row/error/write/result dataclasses, `OzonProductsImportFailure` with exact `error: Exception` and `result: ImportResult | None` public attributes, and narrow exceptions without SQL, filesystem, or FastAPI imports.
...
- [ ] Write RED tests `test_first_logical_key_inserts_revision_one`, `test_equal_hash_returns_duplicate_without_insert`, `test_changed_payload_inserts_revision_two_and_preserves_revision_one`, `test_new_date_and_new_window_each_start_revision_one`, `test_revision_keeps_exact_batch_and_artifact_provenance`, `test_unique_constraint_defends_revision_allocation`, and `test_latest_product_snapshot_orders_by_date_window_then_revision`.
- [ ] Assert correction's `supersedes_snapshot_id == rev1.id`, rev1 row bytes/fields remain unchanged, current lookup returns rev2, and duplicate result returns existing snapshot; DB FK IDs equal supplied provenance. The logical key remains `(product_id, report_generated_on, report_window_days)`. Freeze latest observation ordering for the relevant product as `ORDER BY report_generated_on DESC, report_window_days DESC, revision DESC`: maximum generated date, then maximum window on that date, then maximum revision within that logical key.
- [ ] For `test_latest_product_snapshot_orders_by_date_window_then_revision`, create A `(2026-08-16, 7 days, revision 2)` and B `(2026-08-16, 28 days, revision 1)` for the same product and expect B. Then create C `(2026-08-17, 7 days, revision 1)` and expect C. Date outranks window, window breaks a same-date tie, and revision chooses the current row within the selected logical key; different same-date windows remain separate observations.
...
- [ ] Implement product listing query with a correlated/windowed persistence-only latest-current selection; return already selected freshness inputs, not computed UI prose. `ProductRepository.list_ozon_products()` must use the same latest-current ordering as Task 4: maximum generated date → maximum window on that date → maximum revision within that logical key (`report_generated_on DESC, report_window_days DESC, revision DESC`). Do not mutate on reads.
...
- [ ] Add streaming RED tests: exclusive `.upload-<uuid4>.part` creation; exactly 25 MiB accepted; next byte raises `UploadTooLarge`; SHA-256/size calculated while copying chunks; fsync occurs; generated final name matches `^\d{8}T\d{12}Z-[0-9a-f]{64}\.xlsx$`; `stored_relpath` is POSIX `imports/<name>`; generated collision fails without overwrite; `data/imports` is lazy. Test `safe_original_basename(original_name: str)` exactly: `"report.xlsx"`, `"/tmp/report.xlsx"`, and `"../../secret/report.xlsx"` become `"report.xlsx"`; `r"C:\Users\User\report.xlsx"`, `r"..\..\report.xlsx"`, and `r"\\server\share\report.xlsx"` become `"report.xlsx"`; `""`, `"."`, and `".."` become `"upload.xlsx"`.
...
- [ ] Add failure-carrier RED tests: fatal parser error after batch creation raises `OzonProductsImportFailure` with `result.status == FAILED` and an existing `result.import_batch_id`; persistence failure after batch creation raises the same carrier with a FAILED result; a concurrent lock conflict before batch creation raises it with `result is None`.
- [ ] Add lock RED test: hold `_IMPORT_LOCK`, call import, expect immediate `OzonProductsImportFailure(error=ConcurrentImportConflict(...), result=None)`, no staging/batch mutation; prove lock releases after each success/failure.
...
- [ ] Implement `safe_original_basename(original_name: str)` by treating the input as untrusted display metadata only: `posix_name = PurePosixPath(original_name).name`, then `name = PureWindowsPath(posix_name).name`, with deterministic `"upload.xlsx"` fallback when `name` is empty, `"."`, or `".."`. Do not rely on host `Path`/`PurePath` semantics alone and never use the original filename for a filesystem destination; the generated archive name remains independent.
- [ ] Implement import in the frozen eight-phase lifecycle. Acquire lock before staging; stream/hash/limit/fsync; commit batch and SourceArtifact metadata together before parsing; parse and finish all duplicate-conflict checks before Product mutation; rename exclusively; perform one mutation `transaction()`; compensate only after rollback/close. Never issue SQL, open connections directly, use CAS, or retain a failed path.
- [ ] Freeze failure transport: before an ImportBatch exists, raise `OzonProductsImportFailure(error=<narrow PR3 exception>, result=None)` (including concurrent conflict and service-owned media/size validation). After an ImportBatch exists, finish or compensate it to FAILED, build the final FAILED ImportResult from durable failed state, and raise `OzonProductsImportFailure(error=<narrow PR3 exception>, result=<FAILED ImportResult>)`. Return ImportResult normally only for SUCCESS/PARTIAL_SUCCESS.
- [ ] Build ImportResult from durable summary/artifact and first 50 errors; statuses exactly SUCCESS/PARTIAL_SUCCESS/FAILED. Batch and SourceArtifact metadata are committed together before parse, so every post-batch FAILED ImportResult retains non-optional `source_artifact: SourceArtifact`; use `stored_relpath=None` when archive commit did not succeed. FastAPI must not invent artifact data. Identical input duplicates affect duplicate/warnings only; database duplicates affect duplicate only; at least one skipped recoverable row makes partial; no accepted row is fatal.
...
- [ ] Add POST RED cases: valid synthetic upload returns HTTP 200 SUCCESS; recoverable row returns HTTP 200 PARTIAL_SUCCESS and never 207; missing/wrong multipart field 422; bad schema/wrong report/invalid period/conflicting rows after batch creation return 422 with exact frozen error code/Russian message and `body.result.status == "FAILED"`; wrong request content type or non-`.xlsx` upload 415; >25 MiB 413; held pre-batch lock returns 409 with `body.result is null`; persistence failure returns 500 with `body.result.status == "FAILED"`. Freeze domain/application codes as the exception names converted to upper snake case: `UNSUPPORTED_WORKBOOK`, `WRONG_REPORT_TYPE`, `INCOMPATIBLE_REPORT_SCHEMA`, `INVALID_REPORT_PERIOD`, `CONFLICTING_OBSERVATION_ROWS`, `CONCURRENT_IMPORT_CONFLICT`, `UPLOAD_TOO_LARGE`, `UNSUPPORTED_UPLOAD_MEDIA_TYPE`, and `IMPORT_PERSISTENCE_ERROR`; tests assert the exact applicable value (including `INCOMPATIBLE_REPORT_SCHEMA`, `CONCURRENT_IMPORT_CONFLICT`, and `IMPORT_PERSISTENCE_ERROR`). Assert the deterministic fatal envelope is exactly `{"error": {"code": "<ERROR_CODE>", "message": "<user-facing message>"}, "result": <ImportResult | null>}` and contains no traceback, absolute path, upload content, or internal exception. Success/partial remains a direct ImportResult.
...
- [ ] Add GET products RED cases: owned first; exact Ozon identity; only current revision selected by `report_generated_on DESC, report_window_days DESC, revision DESC`; title/seller/brand; `report_generated_on`, `report_window_days`, `imported_at`; readiness; decimal values (where emitted) JSON strings; no invented boundaries. Include same-product observations on the same date with 7-day and 28-day windows and assert the 28-day context is returned.
...
- [ ] Keep `backend/main.py` thin: adapt upload to the service, serialize typed values (Decimal strings, dates ISO, datetimes UTC ISO), map exact taxonomy/status codes/messages, and use repositories inside `transaction()` for read/PATCH operations. The route must catch `OzonProductsImportFailure`, map `failure.error` to the exact frozen HTTP status/code/message, and include serialized `failure.result` when non-null (otherwise `result: null`) in the frozen envelope. It must not re-read repositories to reconstruct a result, infer counters, reparse the source, expose a traceback, or add a generic global API error framework. Do not parse XLSX, decide revisions/readiness, calculate freshness, or add SQL/routes package.
