...
Import `initialize_database` and `DatabaseMigrationError` from persistence. The existing broad top-level launch failure path remains the final error boundary; the specific migration error type supplies understandable context and preserves its cause. Do not add SQL, PID writing, or FastAPI startup hooks to `launcher.py`.
...
| DB creation without eager imports/backups | Tasks 1, 7 | Directory test and clean portable run assertions |
...
| Authoritative Windows acceptance | Tasks 7, 8 | GitHub Actions `windows-latest` full smoke after user push |
| No new dependencies | Tasks 1, 8 | Exact requirements contract and forbidden dependency scan |
