...
def _visibility(conn, product_id, query_id, observed="2026-02-01T00:00:00+00:00", revision=1, position=5, title="Candidate"):
    ...
    conn.execute("""INSERT INTO search_visibility_snapshots(product_id,search_query_id,cluster_id,observed_at,revision,supersedes_snapshot_id,payload_sha256,import_batch_id,source_artifact_id,imported_at,source_title,seller_name,position,overall_score,promotion_status,cpc_state,cpc_rub,promotion_strategy,cpo_state,cpo_pct,relevance_score,rating,reviews_count,buyer_price_rub,popularity_score,ozon_promotion,delivery_label,delivery_min_days,delivery_max_days,price_index_pct) VALUES (?,?,?,?,?,NULL,?,1,1,?,?,?,?,'1','none','ACTIVE','0','none','DISABLED',NULL,'1',NULL,NULL,'99','1',0,'x',1,1,'1')""",(product_id,query_id,cluster,observed,revision,(str(product_id)+str(revision)).ljust(64,'a')[:64],observed,title,"Seller",position))
