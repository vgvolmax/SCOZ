...
function Add-PortableImports {
    ...
    $code = "from io import BytesIO; from pathlib import Path; import sys; from tests.xlsx_factory import build_ozon_products_workbook,build_ozon_search_visibility_workbook; from backend.application.ozon_products_import import import_ozon_products_xlsx; from backend.application.ozon_search_visibility_import import import_ozon_search_visibility_xlsx; db=Path(sys.argv[1]); data=db.parent; a=import_ozon_products_xlsx(upload=BytesIO(build_ozon_products_workbook()),original_name='products.xlsx',db_path=db,data_dir=data); b=import_ozon_search_visibility_xlsx(upload=BytesIO(build_ozon_search_visibility_workbook()),original_name='visibility.xlsx',db_path=db,data_dir=data); print('PASS')"
    $probe = & (Join-Path $app 'runtime/python.exe') -c $code (Join-Path $data 'scoz.db')
    Assert-True ($probe -contains 'PASS') 'Portable PR3/PR4 import failed'
}
function Assert-PortableImports {
    $code = "import sqlite3,sys; from pathlib import Path; db=Path(sys.argv[1]); c=sqlite3.connect(db); kinds=dict(c.execute('SELECT b.import_kind,a.stored_relpath FROM import_batches b JOIN source_artifacts a ON a.import_batch_id=b.id WHERE b.status IN (\"SUCCESS\",\"PARTIAL_SUCCESS\")')); assert 'ozon_products_xlsx' in kinds and 'ozon_search_visibility_xlsx' in kinds; assert c.execute('SELECT COUNT(*) FROM search_visibility_snapshots').fetchone()[0]>=1; assert all((db.parent/p).is_file() for p in kinds.values()); print('PASS')"
    $probe = Invoke-DbPython $code
    Assert-True ($probe -contains 'PASS') 'PR4 archive did not survive'
}
...
try {
    ...
    $pr4ParserProbe = & $portablePython -c "from io import BytesIO; from tests.xlsx_factory import build_ozon_search_visibility_workbook; from backend.ingestion.ozon_search_visibility_xlsx import parse_ozon_search_visibility_xlsx; data=BytesIO(build_ozon_search_visibility_workbook()); assert len(parse_ozon_search_visibility_xlsx(data).rows)==1; print('PASS')"
    Assert-True ($pr4ParserProbe -contains 'PASS') 'Synthetic PR4 parser smoke failed'
    $recoveryProbe = & $portablePython -c "from pathlib import Path; import sys; from backend.application.ozon_products_import import recover_interrupted_ozon_products_imports; from backend.application.ozon_search_visibility_import import recover_interrupted_ozon_search_visibility_imports; db=Path(sys.argv[1]); data=db.parent; recover_interrupted_ozon_products_imports(db_path=db,data_dir=data); files=set((data/'imports').glob('*.xlsx')); recover_interrupted_ozon_search_visibility_imports(db_path=db,data_dir=data); assert files and files.issubset(set((data/'imports').glob('*.xlsx'))); print('PASS')" (Join-Path $app 'data/scoz.db')
    Assert-True ($recoveryProbe -contains 'PASS') 'Cross-kind archive recovery failed'
