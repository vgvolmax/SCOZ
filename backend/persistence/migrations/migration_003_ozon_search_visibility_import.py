...
def up(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        "CREATE INDEX idx_search_visibility_current ON search_visibility_snapshots"
        "(product_id, search_query_id, cluster_id, observed_at, revision DESC)"
    )
    conn.execute(
        "CREATE INDEX idx_search_visibility_context ON search_visibility_snapshots"
        "(search_query_id, cluster_id, observed_at DESC, product_id, revision DESC)"
    )
    conn.execute(
        "CREATE INDEX idx_search_visibility_product ON search_visibility_snapshots"
        "(product_id, search_query_id, cluster_id, observed_at DESC, revision DESC)"
    )
    conn.execute(
        "CREATE INDEX idx_search_visibility_import_batch_id "
        "ON search_visibility_snapshots(import_batch_id)"
    )
    conn.execute(
        "CREATE INDEX idx_search_visibility_source_artifact_id "
        "ON search_visibility_snapshots(source_artifact_id)"
    )
    conn.execute("ALTER TABLE import_batches ADD COLUMN observed_at TEXT")
    conn.execute("ALTER TABLE import_batches ADD COLUMN search_query_text TEXT")
    conn.execute("ALTER TABLE import_batches ADD COLUMN cluster_name TEXT")
    conn.execute(
        "ALTER TABLE import_batches ADD COLUMN declared_rows INTEGER "
        "CHECK (declared_rows IS NULL OR declared_rows > 0)"
    )
