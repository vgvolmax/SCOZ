...
class LineageRepository:
    ...
    def count_import_history(self) -> int:
        return self._conn.execute("SELECT COUNT(*) FROM import_batches WHERE import_kind IN (?, ?)", ("ozon_products_xlsx", "ozon_search_visibility_xlsx")).fetchone()[0]
    ...
    def _history_item(cls, row: sqlite3.Row) -> ImportHistoryItem:
        products = row["import_kind"] == "ozon_products_xlsx"
        return ImportHistoryItem(
            row["id"], row["source"], row["import_kind"], "OZON_PRODUCTS" if products else "OZON_SEARCH_VISIBILITY", ImportStatus(row["status"]),
            date.fromisoformat(row["report_generated_on"]) if products and row["report_generated_on"] is not None else None,
            row["report_window_days"] if products else None,
            datetime_from_db(row["observed_at"]) if not products and row["observed_at"] is not None else None,
            row["search_query_text"] if not products else None, row["cluster_name"] if not products else None, row["declared_rows"] if not products else None,
            *(0 if row[name] is None else row[name] for name in ("rows_seen","rows_accepted","rows_skipped","duplicate_observations","new_observations","corrected_revisions","warnings_count","row_errors_total")),
            datetime_from_db(row["started_at"]), None if row["finished_at"] is None else datetime_from_db(row["finished_at"]), cls._artifact(row),
        )
