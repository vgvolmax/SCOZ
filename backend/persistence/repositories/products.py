...
class ProductRepository:
    ...
    def count_ozon_products(self) -> int:
        return self._conn.execute("""SELECT COUNT(*)
FROM product_external_identities i
JOIN products p ON p.id=i.product_id
WHERE i.source='ozon' AND i.identity_type='ozon_product_id' AND i.source_account_scope=''
AND EXISTS (SELECT 1 FROM product_snapshots ps WHERE ps.product_id=p.id)""").fetchone()[0]
