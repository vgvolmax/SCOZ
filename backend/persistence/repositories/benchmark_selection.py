...
class BenchmarkSelectionRepository:
    ...
    def list_candidates(self, product_id: int, *, limit: int, offset: int) -> CandidatePage:
        ...
        """
        total = self._conn.execute(cte + "SELECT COUNT(*) FROM agg", (product_id, product_id)).fetchone()[0]
        rows = self._conn.execute(cte + """SELECT r.*,a.query_count,a.cluster_count,a.best_position,i.identity_value,
          ...
          FROM ranked r JOIN agg a ON a.product_id=r.product_id JOIN product_external_identities i ON i.product_id=r.product_id AND i.source='ozon' AND i.identity_type='ozon_product_id' AND i.source_account_scope='' WHERE r.rn=1
    ...
    def _current_revision(self, benchmark_set_id: int) -> BenchmarkSetRevision | None:
        ...
        members = tuple(BenchmarkMember(row["id"],m["product_id"],m["identity_value"]) for m in self._conn.execute("""SELECT bm.product_id,i.identity_value FROM benchmark_members bm JOIN product_external_identities i ON i.product_id=bm.product_id AND i.source='ozon' AND i.identity_type='ozon_product_id' AND i.source_account_scope='' WHERE bm.benchmark_set_revision_id=? ORDER BY CAST(i.identity_value AS INTEGER),bm.product_id""", (row["id"],)))
    ...
    def save_benchmark(self, own_product_id: int, member_product_ids: frozenset[int]) -> BenchmarkCompositionWriteResult:
        ...
        valid=self._conn.execute(f"""SELECT COUNT(DISTINCT p.id) FROM products p JOIN product_external_identities i ON i.product_id=p.id AND i.source='ozon' AND i.identity_type='ozon_product_id' AND i.source_account_scope='' WHERE p.id IN ({marks}) AND p.id<>?""", (*member_product_ids,own_product_id)).fetchone()[0]
